...
var ModeloActualizar = models.CatalogoEspecialidades;
var token = 'QsZ6RbI45mArq9okd4AZBbkMGObdG9MeuTJjKOJzTO3TbMnoQGHzGNu0ZANQ003s';
var host = '69.28.92.27';
var port = 80;

console.log("Migrando datos al modelo CatalogoEspecialidades");

http.get({
    host: host,
    port: port,
    path: '/api/Especialidades/count?access_token='+token
}, function(response) {
    var body = '';
		    ...
		    return new Promise(function(resolve, reject) {
		        // Arbitrary 250ms async method to simulate async process
					...
					http.get({
					    host: host,
					    port: port,
					    path: '/api/Especialidades?filter={"limit":'+num_registros+',"skip":'+inicio+',"fields":["clave","nombre","id","categoriaId"]}&access_token='+token
					}, function(response) {
						    ...
						    response.on('end', function() {
								...
								for (var i = 0; i < resultado.length; i++) {
										...
										datos.push(
										{
										  ...
										  tema_anterior_id : resultado[i].categoriaId
										});
								}
