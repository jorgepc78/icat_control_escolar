	...
	CursosOficiales.exporta_formato = function(idCurso, documento, res, callback) {
            ...
            function(err, resultado) {
					...
					var codigo = {
						...
						"grado_academico"      : CUrsoEncontrado.instructor.gradoAcademico,
						"rfc"			       : CUrsoEncontrado.instructor.rfc,
						"calle"			       : '',
						"numero"			   : '',
						"colonia"			   : '',
						"localidad_instructor" : (CUrsoEncontrado.instructor.localidad_pertenece == undefined ? '' : CUrsoEncontrado.instructor.localidad_pertenece.nombre),
						"municipio_instructor" : (CUrsoEncontrado.instructor.localidad_pertenece == undefined ? '' : CUrsoEncontrado.instructor.localidad_pertenece.municipio),
						"telefono" 			   : CUrsoEncontrado.instructor.telefono,
						"cantidad_total"	   : '',
...
/***********************************************************************************************************************************************/
    ...
    CursosOficiales.cursos_mes = function(id_unidad, anio, meses, cb) {
		...
		/*var sql = 'SELECT a.id_unidad_admtva, a.anio, a.mes, (CASE WHEN a.num_cursos is null THEN 0 ELSE a.num_cursos END) as num_cursos, (CASE WHEN b.num_personas is null THEN 0 ELSE b.num_personas END) as num_personas, (CASE WHEN c.num_personas_terminan is null THEN 0 ELSE c.num_personas_terminan END) as num_personas_terminan FROM '+
				  ...
				  'WHERE cursos_ofertados.inscripcion_curso.pagado > 0 AND cursos_ofertados.cursos_oficiales.estatus IN (5,6) '+codigo+'AND EXTRACT(year FROM fecha_inicio) = '+anio+' GROUP BY id_unidad_admtva, anio, mes) as b '+
