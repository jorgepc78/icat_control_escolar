module.exports = function(ControlProcesos) {
	...
	ControlProcesos.observe('after save', function(ctx, next) {
		...
		function(err, usuarioEncontrado) {
				...
				var Role = ControlProcesos.app.models.Role;

				var idRol = 0;
				if(ctx.instance.proceso == 'PTC')
				{
					if(ctx.instance.accion == 'ENVIO REVISION')
						idRol = 4; // Rol: Depto. Programas de capacitación en central
					else if(ctx.instance.accion == 'PTC RECHAZADO')
						idRol = 9; // Rol: Responsable de capacitación en la unidad
					else if(ctx.instance.accion == 'PTC REVISADO PROGRAMAS')
						idRol = 3; // Rol: Dirección técnica académica en central
				}
				...
				function(err, RolEncontrado) {
						...
						var perfilEncontrado = JSON.parse( JSON.stringify( RolEncontrado ) );

						var arrayIds = [];
						for (var i = 0; i < perfilEncontrado.principals.length; i++) {
							arrayIds.push(perfilEncontrado.principals[i].principalId);
						};
						...
						if(ctx.instance.proceso == 'PTC')
						{
							...
							else if(ctx.instance.accion == 'PTC RECHAZADO')
							{
								condicion = {
									and: [
										{idUsuario: {inq: arrayIds }},
										{idUnidadAdmtva: ctx.instance.idUnidadAdmtvaRecibe},
										{PTCEnvioRevision: true}
									]
								};
							}
						...
						Usuario.find({
							where:  condicion, 
							fields: ['idUsuario', 'nombre', 'email', 'idUnidadAdmtva', 'PTCEnvioRevision','avisosPreaperturaCursos','avisosInscripcion','avisosCierreCursos']
						},
						function(err, usuarioEncontrado2) {
								...
								function(err, PTCencontrado) {
										...
										{					
												...
												mensajes.recibe = mensajes.envia;
										}
										enviaCorreos(ctx.instance.id, mensajes, array_envia, array_unidad, array_recibe);
		...
		function enviaCorreos(idControlProcesos, mensajes, array_envia, array_unidad, array_recibe) {
