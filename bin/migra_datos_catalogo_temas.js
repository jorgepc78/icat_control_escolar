...
var ModeloActualizar = models.CatalogoTemas;
var token = 'YrbGVK7AxM4BG3HoH2pGNdBFF6W9IngENfuC6ApTj69WkCDkG9tnqBgXSo9gBTS4';
var host = '69.28.92.27';
var port = 80;

console.log("Migrando datos al modelo CatalogoTemas");

http.get({
    host: host,
    port: port,
    path: '/api/CatEspecialidads/count?access_token='+token
}, function(response) {
    var body = '';
		    ...
		    return new Promise(function(resolve, reject) {
		        // Arbitrary 250ms async method to simulate async process
					...
					http.get({
					    host: host,
					    port: port,
					    path: '/api/CatEspecialidads?filter={"limit":'+num_registros+',"skip":'+inicio+'}&access_token='+token
					}, function(response) {
						    ...
						    response.on('end', function() {
								...
								for (var i = 0; i < resultado.length; i++) {
										...
										datos.push(
										{
										  fotoInicio  : resultado[i].foto_inicio,
